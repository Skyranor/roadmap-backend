...
15. [Эволюция HTTP (HTTP/2 и HTTP/3)](#15-эволюция-http)
16. [CORS — Междоменные запросы](#16-cors)
17. [Cookies vs JWT](#17-cookies-vs-jwt)
18. [Caching (Кэширование)](#18-caching)
19. [WebSocket](#19-websocket)
20. [Шпаргалка для собеседования](#20-шпаргалка)
21. [Чек-поинт](#21-чек-поинт)
...
## 15. Эволюция HTTP (HTTP/1.1 → HTTP/2 → HTTP/3)

**HTTP/1.1** (1997)
- **Текстовый** протокол.
- **Проблема:** Head-of-line (HOL) blocking. Если браузер запрашивает JS, CSS и картинку по одному TCP-соединению, они скачиваются строго по очереди. Если большой JS-файл "застрял", всё остальное ждёт.
- **Решение в эпоху 1.1:** Браузеры открывали по 6 TCP-соединений к одному домену, чтобы качать файлы параллельно.

**HTTP/2** (2015)
- **Бинарный** протокол.
- **Multiplexing (Мультиплексирование):** Запросы и ответы разбиваются на мелкие фреймы и летят параллельно по **одному** TCP-соединению. Нет HTTP HOL blocking.
- **HPACK:** Сжатие заголовков (в HTTP/1.1 заголовки гоняются текстом в каждом запросе).
- **Проблема TCP HOL blocking:** Мультиплексирование решило затор на уровне HTTP, но если теряется *один TCP-пакет*, операционная система стопарит весь стрим, пока пакет не переотправится.

**HTTP/3** (2022)
- Работает поверх **UDP** (протокол **QUIC**).
- **Нет TCP HOL blocking:** Так как это UDP, потеря пакета для одной картинки не останавливает загрузку CSS.
- **0-RTT Handshake:** QUIC объединяет установку соединения и TLS-шифрование. Быстрее подключается к серверу.

---

## 16. CORS — Междоменные запросы

### Зачем нужен CORS
По умолчанию браузер запрещает фронтенду на `https://omnia.com` делать `fetch` запросы к API на `https://api.omnia.com` или `https://stripe.com`. Это механизм безопасности **Same-Origin Policy (SOP)**. 

**Origin** состоит из: `Протокол + Домен + Порт`. Если хоть что-то отличается — это междоменный запрос (Cross-Origin).

Чтобы браузер разрешил фронтенду прочитать ответ от чужого API, бэкенд должен вернуть специальные заголовки — это и есть **CORS (Cross-Origin Resource Sharing)**.

### Simple Request (Простой запрос)
Если это простой `GET` или `POST` (с `Content-Type: text/plain` или `application/x-www-form-urlencoded`):
1. Браузер отправляет запрос с заголовком `Origin: https://omnia.com`.
2. Бэкенд проверяет и отвечает: `Access-Control-Allow-Origin: https://omnia.com`.
3. Браузер видит совпадение и отдаёт данные JS-коду.

### Preflight Request (Предварительный запрос)
Если запрос сложный (например, `Content-Type: application/json` или есть кастомный заголовок `Authorization`):
1. Браузер **автоматически** отправляет пустой запрос методом **OPTIONS**.
2. В нём спрашивает: `Access-Control-Request-Method: POST`, `Access-Control-Request-Headers: authorization, content-type`.
3. Бэкенд отвечает 200 OK + `Access-Control-Allow-Origin: https://omnia.com` + `Access-Control-Allow-Methods: POST, GET, OPTIONS` + `Access-Control-Allow-Headers: authorization, content-type`.
4. Только получив это разрешение, браузер отправляет реальный `POST` запрос.

> 🔴 **Баг на практике:** Забыли ответить на OPTIONS-запрос (или вернули 404). В Network tab виден красный `CORS error`, хотя сам `POST` даже не был отправлен!

---

## 17. Cookies vs JWT

Оба механизма нужны, потому что HTTP — **stateless** (без состояния). Сервер не помнит предыдущий запрос.

| Характеристика | Cookies (Сессии) | JWT (JSON Web Tokens) |
|---|---|---|
| **Где хранится состояние** | В базе данных / Redis на бэкенде. | В самом токене (stateless). |
| **Что гуляет по сети** | Session ID (случайная строка: `abc123xyz`). | Закодированный JSON с данными пользователя и подписью. |
| **Масштабирование** | Сложнее (нужен общий Redis для всех серверов). | Легче (любой сервер может проверить подпись токена локально). |
| **Отзыв доступа (Logout)** | Легко — удалил сессию из базы, и всё. | Сложно — токен валиден до конца срока действия (TTL), нужен Blacklist. |

### Безопасные Cookies (Must-Have настройки)
Если бэкенд выставляет `Set-Cookie`, он обязан добавить флаги безопасности:
- **HttpOnly** — запрещает чтение куки из `document.cookie` в JS. Защита от **XSS** (кражи сессии через внедрённый скрипт).
- **Secure** — кука передаётся только по HTTPS. Защита от перехвата по сети.
- **SameSite=Lax** или **Strict** — кука не отправляется при кросс-доменных запросах. Защита от **CSRF** атак.

### JWT Паттерн (Access + Refresh)
- **Access Token:** Живёт 15 минут. Отправляется в заголовке `Authorization: Bearer <token>`. Даёт доступ к API.
- **Refresh Token:** Живёт 30 дней. Хранится строго в **HttpOnly Cookie**. Используется только для получения нового Access Token, когда старый протух.

---

## 18. Caching (Кэширование)

Кэширование снимает нагрузку с базы данных и ускоряет ответ для клиента.

### Заголовок Cache-Control
Управляет тем, где и как долго можно кэшировать ответ.
- `Cache-Control: no-cache` — перед использованием кэша браузер ОБЯЗАН спросить бэкенд, не изменился ли файл.
- `Cache-Control: public, max-age=3600` — можно кэшировать всем (включая CDN/Nginx) на 1 час (3600 сек).
- `Cache-Control: private` — можно кэшировать только в браузере конкретного пользователя (например, личный профиль).

### ETag и 304 Not Modified
**ETag** (Entity Tag) — это хэш или версия ресурса. 
1. Сервер возвращает: `ETag: "v1"`. Браузер сохраняет файл и ETag.
2. При следующем запросе браузер шлёт: `If-None-Match: "v1"`.
3. Бэкенд проверяет данные в базе. Если версия всё ещё "v1", он отвечает:
   **304 Not Modified** (без тела ответа!).
4. Браузер берёт файл из своего кэша.
**Итог:** Сэкономили трафик (не гоняли большой JSON по сети).

---

## 19. WebSocket

**HTTP** работает по принципу "запрос-ответ". Сервер не может отправить данные клиенту сам.
Для чатов, лайв-статусов или биржевых котировок (как в Обменке) нужен **WebSocket**.

**Как устанавливается:**
1. Браузер шлёт обычный HTTP GET запрос:
   `Connection: Upgrade`
   `Upgrade: websocket`
2. Бэкенд (если поддерживает) отвечает **101 Switching Protocols**.
3. TCP-соединение остаётся открытым, но HTTP больше не используется.
4. Теперь клиент и сервер могут пушить бинарные/текстовые фреймы друг другу в любой момент (двунаправленная связь).

---

## 20. Шпаргалка для собеседования
...
| TCP vs UDP? | TCP — гарантии (handshake, порядок, retransmission). UDP — быстрый, без гарантий. |
...
| PUT vs PATCH? | PUT = полная замена. PATCH = частичное обновление. |
| Keep-Alive зачем? | Переиспользует TCP+TLS соединение. Экономит 3 RTT на каждом повторном запросе. |
| В чём отличие HTTP/2 от HTTP/1.1? | HTTP/2 бинарный, поддерживает мультиплексирование (запросы параллельно по 1 TCP-соединению) и сжатие заголовков (HPACK). Решает HTTP HOL blocking. |
| Что такое CORS? | Механизм безопасности. Браузер шлёт OPTIONS (preflight) запрос. Сервер должен ответить заголовком Access-Control-Allow-Origin, чтобы разрешить чтение ответа фронтенду на другом домене. |
| HttpOnly флаг в куках? | Запрещает доступ к куке из JavaScript (`document.cookie`). Защищает сессию от кражи через XSS. |
| Как работает ETag? | Сервер отдаёт ETag (хэш/версию). В след. запросе клиент шлёт `If-None-Match`. Если не изменилось, сервер возвращает пустой 304 Not Modified. Экономит трафик. |
| Зачем WebSocket если есть HTTP? | HTTP — это Request/Response (клиент инициатор). WebSocket — двунаправленный постоянный канал, сервер может сам пушить данные клиенту (чаты, лайв). |
...
- [ ] Можешь объяснить путь запроса от `fetch()` до JSON в браузере.
- [ ] Понимаешь TLS handshake и зачем два типа шифрования.
- [ ] Правильно выбираешь status code для любой ситуации (400 vs 422, 401 vs 403).
- [ ] Объясняешь разницу PUT (полная замена) и PATCH (частичное обновление).
- [ ] Знаешь, почему POST не идемпотентен и как работает Idempotency-Key.
- [ ] Знаешь разницу между HTTP/1.1 (HOL blocking) и HTTP/2 (Multiplexing).
- [ ] Можешь объяснить фронтендеру, почему у него в Network tab красный CORS error на OPTIONS-запросе.
- [ ] Понимаешь, зачем флаг `HttpOnly` для сессионной куки.
- [ ] Можешь объяснить механику 304 Not Modified через ETag.

---

## Что дальше — практика

После изучения теории переходи к практическим заданиям. Постман — это хорошо, но для фундамента нужно поработать руками.

1. 🔴 **Ручной HTTP-сервер на Node.js**
   Используй `node:http` (`http.createServer()`). Напиши роуты, которые возвращают:
   - `GET /` → `200 OK`
   - `GET /error` → `500 Internal Server Error`
   - Любой другой URL → `404 Not Found`
2. 🔴 **Анатомия запроса своими руками**
   В том же сервере залогируй в консоль свойства входящего запроса:
   `req.method`, `req.url`, `req.headers`. Сделай запросы из браузера и посмотри, какие заголовки шлёт браузер (User-Agent, Accept).
3. 🟡 **cURL мастерство**
   Сделай запросы к своему серверу через терминал:
   - `curl -X GET http://localhost:3000/`
   - `curl -X POST -H "Content-Type: application/json" -d '{"name":"test"}' http://localhost:3000/`
   - `curl -v http://localhost:3000/` (посмотри сырой текстовый обмен)
4. 🟡 **Анализ трафика (Chrome Network / Wireshark)**
   Открой любой сайт. В Chrome DevTools (Network tab) найди **Timing** вкладку запроса и посмотри время, потраченное на DNS Lookup, Initial connection (TCP) и SSL (TLS).
